...
export async function GET(request: Request) {
  ...
  const limit = 10; // Показывать строго по 10 машин
  ...
  // На Aleado страница передается как pg=1, pg=2...
  ...
  try {
    const response = await fetch(targetUrl, {
      ...
      next: { revalidate: 1800 }, // Кэшируем каждую страницу на 30 минут
    });
    ...
    // Общее количество лотов (из элемента input#rows)
    ...
    $('tr[id^="cell_"]').each((index, element) => {
      if (lots.length >= limit) return false; // Ограничиваем ровно 10 машин на страницу

      const rowId = index + 1;
      const $row = $(element);

      // Извлекаем ссылку и ID лота
